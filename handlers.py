# File: handlers.py (Versi Final Lengkap - Menggabungkan Semua Fitur)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if photos and photos.photos:
            await update.message.reply_photo(photo=photos.photos[0][-1].file_id, caption=user_info, parse_mode='HTML')
        else:
...
async def route_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if command == "menu_vless": # Note: This will be changed to a sub-menu later
        await query.edit_message_text("Masukkan <b>User</b> untuk akun Vless:", parse_mode='HTML')
        return VLESS_GET_USER
    if command == "menu_trojan": # Note: This will be changed to a sub-menu later
    ...
    if command == "close_menu":
        await query.edit_message_text("Menu ditutup. Kirim /menu untuk membuka lagi.")
        return ConversationHandler.END
    else:
        await query.edit_message_text(f"Fitur <b>{command}</b> belum siap.", parse_mode='HTML')
        return ROUTE

# --- FUNGSI GENERIK UNTUK MENANGANI ERROR ---
...
async def ssh_get_ip_limit_and_create(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("⏳ Memproses SSH...")
    try:
...
async def vmess_get_duration(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("<b>Limit IP</b> (0=unlimited):", parse_mode='HTML')
...
async def vmess_get_ip_limit(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("<b>Kuota</b> (GB, 0=unlimited):", parse_mode='HTML')
...
async def vmess_get_quota_and_create(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("⏳ Memproses Vmess...")
    try:
...
async def vless_get_quota_and_create(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("⏳ Memproses Vless...")
    try:
