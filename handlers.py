# File: handlers.py (Versi Final dengan Tombol Sukses & Error)
...
# ... (Fungsi start, menu, admin, route_handler tidak berubah) ...
...
async def handle_script_error(update: Update, context: ContextTypes.DEFAULT_TYPE, error: Exception):
    """Fungsi terpusat untuk menangani dan menampilkan error dari subprocess."""
    if isinstance(error, subprocess.CalledProcessError):
        error_message = error.stdout.strip() or error.stderr.strip()
        # Gunakan keyboard baru
        await update.message.reply_text(f"❌ <b>Gagal Membuat Akun:</b>\n\n<pre>{error_message}</pre>", parse_mode='HTML', reply_markup=keyboards.get_back_to_menu_keyboard())
    else:
        await update.message.reply_text(f"❌ Terjadi kesalahan tak terduga: {error}", reply_markup=keyboards.get_back_to_menu_keyboard())
    context.user_data.clear()
    return ConversationHandler.END

# --- FUNGSI CONVERSATION (SSH) ---
